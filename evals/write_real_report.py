...
def main() -> None:
    ...
    hard = load_sroie(limit=50, cache=True, ocr_only=True)
    if not hard:
        hard = load_sroie(limit=50, cache=False, ocr_only=True)
    sro_h = run_extract_bench(hard) if hard else {"cases": 0, "micro_field_accuracy": 0.0, "per_field": {}}
    ...
    lines += _section(
        ...
        [
            ...
            "This track includes an explicit labeled footer (`Vendor:` / `Total:`) — **format-assisted**; "
            "use Track B2 for honest OCR difficulty.",
        ],
    )
    lines += _section(
        ...
        sro_h,
        [
            "Same receipts with **assist footer stripped**. Invoice # not scored "
            "(synthetic `SROIE-*` ids only exist in the footer).",
    ...
    print(
        "synthetic",
        sr["micro_field_accuracy"],
        "sroie",
        sro.get("micro_field_accuracy"),
        "sroie_hard",
        sro_h.get("micro_field_accuracy"),
        "cord",
        cr.get("micro_field_accuracy"),
    )
